...
export default function CommunicationsPage() {
    ...
    // Determine standard role (profile hook lowercases role strings)
    const userRoleLower = currentUser?.role?.toLowerCase();
    let role = UserRole.STUDENT;
    if (userRoleLower === 'faculty') role = UserRole.FACULTY;
    else if (userRoleLower === 'admin') role = UserRole.ADMIN;
    else if (userRoleLower === 'hr') role = UserRole.HR;

    // Only block if we have loaded the user AND they are not admin/hr
    if (currentUser && userRoleLower !== 'admin' && userRoleLower !== 'hr') {
        return (
            <DashboardLayout userRole={role} userName={currentUser?.fullName || 'User'} notifications={[]}>
                <div className="flex flex-col items-center justify-center min-h-[60vh] text-text-secondary">
                    ...
                    <p className="text-base font-bold text-text-primary">Admin Access Only</p>
                    <p className="text-sm text-text-muted mt-1">Broadcast management is restricted to administrators.</p>
                </div>
            </DashboardLayout>
        );
    }
    ...
    return (
        <DashboardLayout userRole={role} userName={currentUser?.fullName || 'Admin'} notifications={[]}>
