...
export const getActivityReports = async (req: Request, res: Response<ApiResponse>) => {
  try {
    ...
    // Open escalations
    const escalatedReportsCount = await prisma.escalation.count({
    ...
    return sendError(res, error.message || 'Failed to fetch activity reports');
  }
};
...
export const getWorkloadAnalytics = async (req: Request, res: Response<ApiResponse>) => {
  try {
    ...
    const completedSessions = await prisma.classsession.count({ where: { status: 'COMPLETED' } });
    ...
    const reportsTodayCount = await prisma.dailyactivityreport.count({
      where: {
    ...
    const metrics = {
      ...
      pendingTeachingLoads: await prisma.teachingload.count({ where: { status: 'PENDING' } }),
      leavesAwaitingHR: await prisma.leaverequest.count({ where: { status: 'PENDING_HR' } }),
